...
class Neo4jClient:
    ...
    def search_entities(self, keyword: str, entity_type: str = None) -> list[dict]:
        ...
        params = {"keyword": f"(?i).*{keyword}.*"}
        if entity_type:
            params["type"] = entity_type
        return self._run(
            f"""
            MATCH (e:Entity)
            WHERE e.name =~ $keyword {type_filter}
            RETURN e.name AS `e.name`, e.type AS `e.type`, e AS e
            LIMIT 20
            """,
            params,
        )
    ...
    def link_document_entity(self, doc_id: str, entity_name: str) -> None:
        if not self._available:
            return
        self.upsert_entity(entity_name, "Concept")
        self._run(
            """
    ...
    def search_documents(self, keyword: str, limit: int = 5) -> list[dict]:
        return self._run(
            """
            MATCH (d:Document)
            WHERE d.title =~ $keyword OR d.snippet =~ $keyword
            RETURN d.doc_id AS doc_id, d.title AS title, d.snippet AS snippet
            LIMIT $limit
            """,
            {"keyword": f"(?i).*{keyword}.*", "limit": limit},
        )
    ...
    def graph_retrieve(
        self, entity_names: list[str], relation_types: list[str] = None
    ) -> list[dict]:
        relation_filter = ""
        ...
        return self._run(
            ...
            WHERE e.name IN $names {relation_filter}
    ...
    def log_conversation(
        self, session_id: str, user_msg: str, agent_msg: str, entities: list[str] = None
    ) -> None:
        if not self._available:
            return
        ...
        self._run(
            ...
            CREATE (c)-[:CONTAINS]->(m)
            """,
            {"session_id": session_id, "user_msg": user_msg, "agent_msg": agent_msg},
        )
        self._run(
            """
            MATCH (m:Message)
            WHERE m.user = $user_msg AND m.agent = $agent_msg
            WITH m
            UNWIND $entities AS entity_name
            MATCH (e:Entity {name: entity_name})
            MERGE (m)-[:MENTIONS]->(e)
            """,
            {"user_msg": user_msg, "agent_msg": agent_msg, "entities": entities},
        )
    ...
    def clear_session(self, session_id: str) -> int:
        ...
        results = self._run(
            """
            MATCH (c:Conversation {session_id: $session_id})-[:CONTAINS]->(m:Message)
            DETACH DELETE m, c
